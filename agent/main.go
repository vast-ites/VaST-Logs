...
func main() {
	...
	if cfg.LogConfig.Mode != "none" {
		if cfg.LogConfig.Mode == "selected" {
			for _, path := range cfg.LogConfig.SelectedLogs {
				...
				if strings.Contains(path, "caddy") {
					sourceType = "caddy"
				}
				if strings.Contains(path, "traefik") {
					sourceType = "traefik"
				}
				if strings.Contains(path, "pm2") {
					sourceType = "pm2"
				}
				...
				if strings.Contains(path, "mongodb") || strings.Contains(path, "mongod") {
					sourceType = "mongodb"
				}
				if strings.Contains(path, "postgresql") || strings.Contains(path, "postgres") {
			...
			if cfg.Collectors.Apache {
				logs = append(logs, discovery.FindServiceLogs("apache")...)
			}
			if cfg.Collectors.Nginx {
				logs = append(logs, discovery.FindServiceLogs("nginx")...)
			}
			if cfg.Collectors.PM2 {
				logs = append(logs, discovery.FindServiceLogs("pm2")...)
			}
	...
	for _, l := range logs {
		// Check if explicitly disabled via collectors configuration
		// We use strings.Contains to catch both source type and the path itself
		if !cfg.Collectors.Apache && (l.SourceType == "apache" || strings.Contains(strings.ToLower(l.Path), "/apache2/") || strings.Contains(strings.ToLower(l.Path), "/httpd/")) {
			continue
		}
		if !cfg.Collectors.Nginx && (l.SourceType == "nginx" || strings.Contains(strings.ToLower(l.Path), "/nginx/")) {
			continue
		}
		if !cfg.Collectors.PM2 && (l.SourceType == "pm2" || strings.Contains(strings.ToLower(l.Path), ".pm2/logs") || strings.Contains(strings.ToLower(l.Path), "/pm2/")) {
			continue
		}
	...
	go func() {
		if procs, err := procCol.Collect(); err == nil {
			senderClient.SendProcesses(procs)
		}
		if fw, err := fwCol.Collect(); err == nil {
			senderClient.SendFirewall(fw)
		}
	}()
...
func runSetup() {
	...
	if modeStr == "2" {
		...
		if collectors.Nginx {
			candidates = append(candidates, discovery.FindServiceLogs("nginx")...)
		}
		if collectors.Apache {
			candidates = append(candidates, discovery.FindServiceLogs("apache")...)
		}
		if collectors.PM2 {
			candidates = append(candidates, discovery.FindServiceLogs("pm2")...)
		}
...
func startPM2Collector(client *sender.Client) {
	fmt.Println(">> PM2 Collection Enabled: Starting PM2 metrics collection (30s interval)")
